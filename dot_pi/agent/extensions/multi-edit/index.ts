...
import type { ExtensionAPI, EditToolDetails, Theme } from "@mariozechner/pi-coding-agent";
...
function applyBgToRange(
	...
	while (i < ansiStr.length) {
		...
		if (visIdx === rangeStart && !inRange) { result += bg; inRange = true; }
		if (visIdx === rangeEnd && inRange) { result += restoreBg; inRange = false; }
		result += ansiStr[i];
		visIdx++;
		i++;
	}
...
function renderIntraLineDiff(
	...
	const addChangedRange = (
		...
		const wsLen = skipLeadingWhitespace ? (value.match(/^(\s*)/)?.[1] || "").length : 0;
		if (len > wsLen) ranges.push({ start: pos + wsLen, end: pos + len });
		return pos + len;
	};
	for (const part of parts) {
		const isFirst = removedRanges.length === 0 && addedRanges.length === 0;
		if (part.removed) { oldPos = addChangedRange(removedRanges, oldPos, part.value, isFirst); }
		else if (part.added) { newPos = addChangedRange(addedRanges, newPos, part.value, isFirst); }
		else { oldPos += part.value.length; newPos += part.value.length; }
	}
	for (let r = removedRanges.length - 1; r >= 0; r--) {
		removedLine = applyBgToRange(removedLine, removedRanges[r].start, removedRanges[r].end, removedWordBg, removedLineBg);
	}
	for (let r = addedRanges.length - 1; r >= 0; r--) {
		addedLine = applyBgToRange(addedLine, addedRanges[r].start, addedRanges[r].end, addedWordBg, addedLineBg);
	}
...
function renderDiff(diffText: string, theme: Theme, lang?: string): string {
	...
	while (i < lines.length) {
		...
		if (!parsed) { result.push(theme.fg("toolDiffContext", lines[i])); i++; continue; }
		if (parsed.prefix === "-") {
			const rem = collectDiffLines(lines, i, "-");
			const add = collectDiffLines(lines, rem.i, "+");
			i = add.i;
			if (rem.collected.length === 1 && add.collected.length === 1) {
				const r = rem.collected[0], a = add.collected[0];
				const rPlain = tabs(r.content), aPlain = tabs(a.content);
				const { removedLine, addedLine } = renderIntraLineDiff(
					rPlain, aPlain, hl(rPlain), hl(aPlain),
					REMOVED_WORD_BG, ADDED_WORD_BG, REMOVED_LINE_BG, ADDED_LINE_BG,
				);
				result.push(fmtDiffLine(theme, "toolDiffRemoved", "-", r.lineNum, removedLine));
				result.push(fmtDiffLine(theme, "toolDiffAdded", "+", a.lineNum, addedLine));
			} else {
				for (const r of rem.collected) result.push(fmtDiffLine(theme, "toolDiffRemoved", "-", r.lineNum, hl(tabs(r.content))));
				for (const a of add.collected) result.push(fmtDiffLine(theme, "toolDiffAdded", "+", a.lineNum, hl(tabs(a.content))));
			}
		} else if (parsed.prefix === "+") {
			result.push(fmtDiffLine(theme, "toolDiffAdded", "+", parsed.lineNum, hl(tabs(parsed.content))));
			i++;
		} else {
			result.push(fmtDiffLine(theme, "toolDiffContext", " ", parsed.lineNum, hl(tabs(parsed.content))));
			i++;
		}
	}
	return result.join("\n");
...
export default function (pi: ExtensionAPI) {
	...
	pi.registerTool({
		...
		renderCall(args, theme) {
			...
			return {
				invalidate() {},
				render(_width: number) { return [title]; },
			} as any;
		},

		renderResult(result, { isPartial }, theme) {
			const { details, isError } = result as { details?: EditToolDetails; isError?: boolean };
