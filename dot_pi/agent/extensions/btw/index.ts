...
type LoadedExtension = { handlers?: Map<string, unknown[]> };
type ExtensionsResult = Awaited<ReturnType<typeof discoverAndLoadExtensions>>;

// Load request-shaping extensions (e.g. pi-claude-code-use) so the side
// session's `before_provider_request` hook fires exactly like the main
// session. Without this the side session sends raw, un-masked payloads to the
// OAuth subscription endpoint. Empty cwd/agentDir keep the load minimal and
// prevent re-loading btw itself.
async function loadRequestHookExtensions(): Promise<ExtensionsResult> {
  const empty = mkdtempSync(join(tmpdir(), "btw-ext-"));
  ...
  try {
    ...
    // No readable settings -> side session runs without request hooks.
  }

  const loaded = await discoverAndLoadExtensions(packagePaths, empty, empty);
  return {
    ...loaded,
    extensions: loaded.extensions.filter(
      (e: LoadedExtension) =>
        (e.handlers?.get("before_provider_request")?.length ?? 0) > 0,
    ),
  };
}
...
async function createBtwResourceLoader(
  ...
  const extensionsResult = await loadRequestHookExtensions();
