...
> => {
  const actions: SelectItem[] = [
    ...(options.canDiff ? [{ value: "diff", label: "Diff in Neovim" }] : []),
...
const openPath = async (
  ...
  if (editorCmd) {
    ...
    const result = await pi.exec(editor, [...editorArgs, target.resolvedPath]);
    if (result.code !== 0) {
      const errorMessage =
        result.stderr?.trim() ||
        `Failed to open ${target.displayPath} in $EDITOR`;
...
const editPath = async (
  ...
  if (!editorCmd) {
    ctx.ui.notify("No editor configured. Set $EDITOR or $VISUAL.", "warning");
    return;
  }
...
const openDiff = async (
  ...
  const status = await ctx.ui.custom<number>((tui, theme, _kb, done) => {
    const message = new Text(
      theme.fg("dim", `Opening Neovim diff for ${target.displayPath}...`),
    );

    queueMicrotask(() => {
      tui.stop();
      try {
        const result = spawnSync("nvim", ["-d", tmpFile, workingPath], {
          stdio: "inherit",
          cwd: gitRoot,
        });
