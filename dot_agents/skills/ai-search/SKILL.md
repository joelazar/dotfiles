...
- `--provider codex|claude-code|openai-cli`
- `--model <model-id>` (defaults: `codex` → `gpt-5.5` with low reasoning, `claude-code` → `claude-haiku-4-5` with low thinking budget, `openai-cli` → `gpt-5.3-chat-latest` or `$OPENAI_CLI_MODEL`)
- `--timeout <ms>`
- `--json`
